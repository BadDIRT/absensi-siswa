...
@section('title', 'ABSENSIKU - Edit Data Guru')
...
<header class="sticky top-0 z-40 w-full bg-white/10 backdrop-blur-2xl border-b border-white/10 shadow-xl 
    ...
    <div class="flex items-center gap-3">
        ...
        <h1 class="text-xl sm:text-2xl font-bold drop-shadow">Edit Data Guru</h1>
    </div>
</header>
@endsection
...
<div class="bg-white/10 backdrop-blur-2xl border border-white/10 rounded-3xl shadow-2xl 
            p-6 sm:p-8 max-w-3xl mx-auto transition-all duration-500 
            hover:shadow-blue-500/30 hover:shadow-2xl hover:-translate-y-1">

    <h2 class="text-lg sm:text-2xl font-bold mb-6">Form Edit Guru</h2>

    <form method="POST" action="{{ route('admin.teachers.update', $teacher->id) }}" class="space-y-5">
        @csrf
        @method('PUT')
        ...
        <div>
            ...
            <input type="text" name="name"
                ...
                class="w-full mt-2 p-3 rounded-xl bg-white/5 border border-white/20 
                       backdrop-blur-xl text-white placeholder-white/50
                       transition duration-300 hover:bg-white/20 focus:bg-white/30 
        ...
        <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
            <div>
                ...
                <input type="text" name="nip"
                    ...
                    class="w-full mt-2 p-3 rounded-xl bg-white/5 border border-white/20 
                           backdrop-blur-xl text-white placeholder-white/50 transition 
        ...
        <div>
            ...
            <select name="gender"
                ...
                <option value="L" class="text-black" {{ old('gender', $teacher->gender) == 'L' ? 'selected' : '' }}>
                    Laki-laki
                </option>
                <option value="P" class="text-black" {{ old('gender', $teacher->gender) == 'P' ? 'selected' : '' }}>
                    Perempuan
                </option>
        ...
        <div>
            ...
            <textarea name="address" rows="3"
                ...
                class="w-full mt-2 p-3 rounded-xl bg-white/5 border border-white/20 
                       backdrop-blur-xl text-white placeholder-white/50 transition 
                       hover:bg-white/20 focus:bg-white/30 focus:ring-2 focus:ring-blue-400">{{ old('address', $teacher->address) }}</textarea>
        ...
        <div class="flex flex-col sm:flex-row gap-3 pt-4">
            ...
            <a href="{{ route('admin.teachers.index') }}"
                class="px-5 py-3 bg-white/20 hover:bg-white/30 border border-white/30 rounded-xl 
                       font-semibold text-center backdrop-blur-xl transition duration-300 
            ...
            <button type="submit"
                class="px-5 py-3 bg-blue-600/60 hover:bg-blue-600/80 border border-white/20 
                       rounded-xl font-semibold backdrop-blur-xl w-full sm:w-auto transition duration-300 
