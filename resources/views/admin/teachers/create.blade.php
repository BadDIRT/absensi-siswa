    ...
    <div class="flex-1 flex flex-col transition-all duration-300">
      ...
      <header 
        ...
        <div class="flex items-center gap-3">
          ...
          <h1 class="text-2xl font-bold text-white drop-shadow">Tambah Guru</h1>
        </div>
      ...
      <main class="p-6 flex-1 mt-4">
        <div class="bg-white/30 backdrop-blur-xl border border-white/20 rounded-2xl shadow-xl p-8 max-w-2xl mx-auto text-white transition-all duration-300 hover:shadow-2xl">
          <h2 class="text-3xl font-bold mb-8 text-center">Form Tambah Guru</h2>
          ...
          <form method="POST" action="{{ route('teachers.store') }}" class="space-y-5">
            ...
            <div class="grid grid-cols-2 gap-4">
              <div>
                ...
                <input type="text" name="nip" value="{{ old('nip') }}" placeholder="Masukkan NIP" class="w-full p-3 rounded-xl bg-white/40 border border-white/60 text-black placeholder-gray-600 focus:ring-2 focus:ring-blue-500 focus:scale-[1.02] transition">
                @error('nip') <p class="text-red-400 text-sm mt-1">{{ $message }}</p> @enderror
              </div>

              <div>
                <label class="block mb-2 font-semibold">No. Telepon</label>
                <input type="text" name="phone_number" value="{{ old('phone_number') }}" placeholder="Masukkan No. Telepon" class="w-full p-3 rounded-xl bg-white/40 border border-white/60 text-black placeholder-gray-600 focus:ring-2 focus:ring-blue-500 focus:scale-[1.02] transition">
            ...
            <div>
              ...
              <textarea name="address" rows="3" placeholder="Masukkan alamat lengkap guru" class="w-full p-3 rounded-xl bg-white/40 border border-white/60 text-black placeholder-gray-600 focus:ring-2 focus:ring-blue-500 focus:scale-[1.02] transition">{{ old('address') }}</textarea>
