    ...
    <div class="max-w-xl mx-auto bg-white/20 backdrop-blur-xl rounded-2xl p-6 shadow-lg">
      <form method="POST" action="{{ route('students.update', $student->id) }}" class="space-y-4">
        ...
        <div>
          ...
          <input type="text" name="name" value="{{ old('name', $student->name) }}" required class="w-full p-2 rounded text-black">
        </div>
        ...
        <div>
          ...
          <input type="date" name="date_of_birth" value="{{ old('date_of_birth', $student->date_of_birth) }}" required class="w-full p-2 rounded text-black">
        </div>

        <div>
          <label class="block mb-1 font-semibold">NISN</label>
          <input type="text" name="nisn" value="{{ old('nisn', $student->nisn) }}" required class="w-full p-2 rounded text-black">
        </div>

        <div>
          <label class="block mb-1 font-semibold">NIPD</label>
          <input type="text" name="nipd" value="{{ old('nipd', $student->nipd) }}" required class="w-full p-2 rounded text-black">
        </div>
        ...
        <div>
          ...
          <select name="class_id" required class="w-full p-2 rounded text-black">
            @foreach($classes as $c)
              <option value="{{ $c->id }}" {{ $student->class_id == $c->id ? 'selected' : '' }}>{{ $c->grade }}</option>
            @endforeach
          </select>
        </div>
        ...
        <div>
          ...
          <select name="department_id" required class="w-full p-2 rounded text-black">
            @foreach($departments as $d)
              <option value="{{ $d->id }}" {{ $student->department_id == $d->id ? 'selected' : '' }}>{{ $d->name }}</option>
            @endforeach
          </select>
        </div>

        <button type="submit" class="w-full bg-yellow-500 hover:bg-yellow-600 py-2 rounded-xl font-semibold mt-4">Update</button>
      </form>
    </div>
  </main>
