        ...
        <div class="flex-1 flex flex-col transition-all duration-300">
            ...
            <header class="sticky top-0 z-40 w-full bg-white/10 backdrop-blur-2xl border-b border-white/10 shadow-xl 
                ...
                <a href="{{ route('students.index') }}"
                   class="px-4 py-2 bg-white/10 hover:bg-white/20 border border-white/20 rounded-xl backdrop-blur-xl
                          transition-all duration-300 hover:scale-105 hover:shadow-lg font-semibold text-white">
                    Kembali
                </a>
            </header>
            ...
            <main class="p-2 sm:p-6 flex-1 mt-4">
                <div class="bg-white/10 backdrop-blur-2xl border border-white/10 rounded-3xl shadow-2xl 
                            p-6 sm:p-8 transition duration-300 hover:shadow-blue-500/30 hover:-translate-y-1 max-w-2xl mx-auto">

                    <h2 class="text-xl sm:text-2xl font-bold mb-6">Form Edit Siswa/i</h2>
                    ...
                    <form method="POST" action="{{ route('students.update', $student->id) }}" class="space-y-5">
                        ...
                        <!-- NAMA -->
                        <div>
                            <label class="font-semibold text-sm">Nama Siswa/i</label>
                            <input type="text" name="name" value="{{ old('name', $student->name) }}"
                                   placeholder="Masukkan nama lengkap"
                                   class="w-full mt-2 p-3 rounded-xl bg-white/5 border border-white/20 text-white placeholder-gray-300 backdrop-blur-xl
                                          transition duration-300 hover:bg-white/20 focus:bg-white/30 focus:ring-2 focus:ring-blue-400">
                            @error('name')
                                <p class="mt-2 text-red-400 text-sm drop-shadow-[0_0_6px_rgba(255,0,0,0.8)]">{{ $message }}</p>
                            @enderror
                        </div>
                        ...
                        <div>
                            ...
                            <select name="class_id"
                                ...
                                @foreach($classes as $class)
                                    <option class="text-black" value="{{ $class->id }}" {{ old('class_id', $student->class_id) == $class->id ? 'selected' : '' }}>
                                        {{ $class->grade }}
                                    </option>
                                @endforeach
                            </select>
                        ...
                        <div>
                            ...
                            <select name="department_id"
                                ...
                                @foreach($departments as $d)
                                    <option class="text-black" value="{{ $d->id }}" {{ old('department_id', $student->department_id) == $d->id ? 'selected' : '' }}>
                                        {{ $d->name }}
                                    </option>
                                @endforeach
                            </select>
                        ...
                        <div class="flex flex-col sm:flex-row gap-3 pt-4">
                            ...
                            <a href="{{ route('students.index') }}"
                               class="px-5 py-3 bg-white/20 hover:bg-white/30 border border-white/30 rounded-xl font-semibold 
                                      backdrop-blur-xl text-center transition duration-300 hover:-translate-y-1 hover:shadow-lg">
                                Kembali
                            </a>

                            <button type="submit"
                                    class="px-5 py-3 bg-blue-600/60 hover:bg-blue-600/80 border border-white/20 rounded-xl font-semibold 
                                           backdrop-blur-xl w-full sm:w-auto transition duration-300 transform hover:-translate-y-1 hover:shadow-lg hover:shadow-blue-500/30">
                                Update Data
                            </button>
