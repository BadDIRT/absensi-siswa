...
<head>
    ...
    <title>ABSENSIKU - Tambah Kelas</title>
        ...
        <div class="flex-1 flex flex-col transition-all duration-300">
            ...
            <header class="sticky top-0 z-40 w-full bg-white/10 backdrop-blur-2xl border-b border-white/10 shadow-xl p-3 sm:p-4 flex items-center justify-between rounded-2xl">
                <div class="flex items-center gap-3">
                    ...
                    <h1 class="text-xl sm:text-2xl font-bold drop-shadow">Tambah Data Kelas</h1>
                </div>
            </header>
            ...
            <main class="p-2 sm:p-6 space-y-6 flex-1 mt-4">
                ...
                <div class="bg-white/10 backdrop-blur-2xl border border-white/10 rounded-3xl shadow-2xl p-6 sm:p-8 
                    ...
                    <h2 class="text-lg sm:text-2xl font-bold mb-6">Form Tambah Kelas</h2>
                    ...
                    <form method="POST" action="{{ route('classes.store') }}" class="space-y-5">
                        ...
                        <div>
                            ...
                            <select name="grade"
                                ...
                                <option value="" class="text-black">Pilih Tingkat</option>
                                <option class="text-black" value="10" {{ old('grade')=='10' ? 'selected' : '' }}>10</option>
                                <option class="text-black" value="11" {{ old('grade')=='11' ? 'selected' : '' }}>11</option>
                                <option class="text-black" value="12" {{ old('grade')=='12' ? 'selected' : '' }}>12</option>
                            </select>

                            @error('grade')
                                <p class="mt-2 text-red-400 drop-shadow-[0_0_6px_rgba(255,0,0,0.8)] text-sm font-semibold" x-transition.opacity>
                                    {{ $message }}
                                </p>
                            @enderror
                        </div>
                        ...
                        <!-- WALI KELAS -->
                        <div>
                            <label class="font-semibold text-sm">Wali Kelas (Opsional)</label>
                            <select name="teacher_id"
                                class="w-full mt-2 p-3 rounded-xl bg-white/5 border border-white/20 backdrop-blur-xl text-white
                                       transition duration-300 hover:bg-white/20 focus:bg-white/30 focus:ring-2 focus:ring-blue-400">
                                <option class="text-black" value="">-- Pilih Wali Kelas --</option>
                                @foreach($teachers as $t)
                                    <option class="text-black" value="{{ $t->id }}" {{ old('teacher_id')==$t->id ? 'selected' : '' }}>
                                        {{ $t->name }}
                                    </option>
                                @endforeach
                            </select>
