...
@section('title', 'ABSENSIKU - Edit Jadwal')
...
<header class="sticky top-0 z-40 w-full bg-white/10 backdrop-blur-2xl border-b border-white/10 shadow-xl p-3 sm:p-4 flex items-center justify-between rounded-2xl">
    <div class="flex items-center gap-3">
        ...
        <h1 class="text-xl sm:text-2xl font-bold drop-shadow">Edit Jadwal Pelajaran</h1>
    </div>
</header>
@endsection
...
<div class="bg-white/10 backdrop-blur-2xl border border-white/10 rounded-3xl shadow-2xl p-6 sm:p-8 
    ...
    <h2 class="text-lg sm:text-2xl font-bold mb-6">Form Edit Jadwal</h2>

    <form action="{{ route('timetables.update', $timetable->id) }}" method="POST" class="space-y-5">
        @csrf
        @method('PUT')
            ...
            <select name="day"
                ...
                @foreach (['Senin','Selasa','Rabu','Kamis','Jumat','Sabtu'] as $hari)
                    <option class="text-black"
                        value="{{ $hari }}"
                        {{ old('day', $timetable->day) == $hari ? 'selected' : '' }}>
                        {{ $hari }}
                    </option>
                @endforeach
            </select>
        ...
        <div>
            ...
            <select name="subject_id"
                class="w-full mt-2 p-3 rounded-xl bg-white/5 border border-white/20 backdrop-blur-xl text-white
                       transition duration-300 hover:bg-white/20 focus:bg-white/30 focus:ring-2 focus:ring-blue-400">
                <option value="" class="text-black" >-- Pilih Mapel --</option>
            ...
            <select name="class_id"
                ...
                @foreach ($classes as $c)
                    <option class="text-black" value="{{ $c->id }}"
                        {{ old('class_id', $timetable->class_id) == $c->id ? 'selected' : '' }}>
                        {{ $c->grade }}
                    </option>
                @endforeach
            </select>
